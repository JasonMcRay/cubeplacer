...
public class PlaceCubeTask extends TimerTask {
    ...
    public void run(){
        ...
        if(!triggeredByCommand){
            ...
            user = "test";
            p = Minecraft.getMinecraft().thePlayer.getName();
        }
        ...
        player.getEntityWorld().spawnEntityInWorld(new EntityItem(player.getEntityWorld(), player.posX, player.posY, player.posZ, new ItemStack(Item.getItemFromBlock(CCubesBlocks.CHANCE_CUBE), failedPlacements)));
    }
